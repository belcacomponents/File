...
// TODO вынести в отдельный пакет вместе с расширенными функциями по управлению файлов:
// массовая обработка, генерация изображений и т.п.
// При этом текущий класс все равно должен быть отдельно, как дополнение к FGen

// TODO можно вообще все функции вынести с отдельный класс, а потом унаследовать
// или с помощью трейта.
...
class SimpleImageHandler extends FHandler
{
    ...
    // По умолчанию в качестве префикса используется исходная директория
    // с файлом.
    // При необходимости, например, необходимо полность сменить хранилище,
    // можно передать дополнительный параметр в функцию.
    /**
    ...
    public function resize($filename, $options = [])
    {
        ...
        if (isset($options['directory_prefix_pattern']) || isset($options['directory_pattern'])) {
            ...
            $finalDirectory = $directoryPrefix.'/'.$this->makeDirectory($options['directory_pattern'] ?? null, $options);
        } else {
    ...
    public function thumbnail($filename, $options = [])
    {
        ...
        $quality = $this->initQuality($options['quality'] ?? null);
        $newFile = $this->makeFilename($originalFile, $options['filename_pattern'] ?? null, $options, $this->getFinalDirectory());
        $this->saveImage($img, $newFile, $quality);
        ...
        return [
            ...
            'mime' => $img->mime(),
            'fullname' => $newFile,
            'relativename' => $filename, // TODO не тот файл
            'filename' => $pathinfo['basename'],
            'shortname' => $pathinfo['filename'],
            'final_directory' => $this->getFinalDirectory(),
            'source_directory' => $this->getSourceDirectory(),
    ...
    public function signature($filename, $options = [])
    {
        ...
        $pathinfo = pathinfo($newFile);
        // Нанести изображение и сжать или наоборот
        // + Изменить масштаб водяного знака по настройкам
    ...
    public function insertImage(\Intervention\Image\Image &$img, $filename, $position, $size = null, $offset = null)
    {
        ...
        $mark = $this->manager->make($filename);
